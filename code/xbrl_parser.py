...
class XBRLCorporateFilingParser:
    ...
    def calcXbrl(self, formula, context):
        pattern = "[\[\]]"
        formulaList = re.split(pattern, formula)
        formulaString = ""
        operands = self.attribute_df["attribute"].tolist()
    ...
    def get(self, attribute, context):
        ...
        if x.attribute_type[0] == 'raw_xbrl':
            attributeName = self.attribute_df.loc[(self.attribute_df.attribute == attribute) &
                                                  (self.attribute_df.result_type == self.resultType)] \
            ...
            resultValue = self.calcXbrl(x.value_expr[0], context)
...
if __name__ == '__main__':
    ...
    value = xbrl_parser.get('EBIT', 'FourD')
    print('Value: ', value)
